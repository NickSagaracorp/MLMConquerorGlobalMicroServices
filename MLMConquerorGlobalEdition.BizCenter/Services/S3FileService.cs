...
/// <summary>
/// S3 uploader used by the Profile photo flow. Mirrors the implementation in
/// SignupAPI so both APIs hit the same bucket using the same key convention.
/// In environments where S3 is not configured (no <c>AWS:S3:BucketName</c>),
/// uploads are silently skipped and the caller receives an empty URL — useful
...
public class S3FileService : IS3FileService
{
    ...
    {
        if (!_isConfigured) return string.Empty;
