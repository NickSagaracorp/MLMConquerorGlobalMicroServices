...
///
/// SI ESTA PRUEBA SE PONE EN ROJO: lo que hay que quitar es la dependencia, no la prueba. Si algo
/// de ClientCore necesita de verdad el HttpContext, es que ese algo no era compartible y su sitio
/// es SharedComponents — igual que <c>HttpContextAccessTokenProvider</c>, que se quedó allí
...
public class ClientCoreNoAspNetCoreTests
{
    ...
    [Fact]
    public void ClientCore_NoReferenciaNingunEnsambladoDeAspNetCore()
    {
        var aspNetCoreReferences = ClientCoreAssembly
            .GetReferencedAssemblies()
            .Select(a => a.Name ?? string.Empty)
            .Where(name => name.StartsWith("Microsoft.AspNetCore", StringComparison.OrdinalIgnoreCase))
            .ToArray();

        aspNetCoreReferences.Should().BeEmpty(
            "ClientCore tiene que poder compilarse dentro de una aplicación MAUI, y una MAUI no " +
            "puede referenciar ASP.NET Core. Lo que necesite HttpContext va en SharedComponents, " +
            "detrás de una abstracción como IAccessTokenProvider.");
    }
