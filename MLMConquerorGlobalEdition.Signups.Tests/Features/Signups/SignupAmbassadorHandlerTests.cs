...
public class SignupAmbassadorHandlerTests
{
    ...
    [Fact]
    public async Task Handle_WhenApplicantIsUnder18_ReturnsUnderage()
    {
        ...
        req.DateOfBirth = FixedNow.AddYears(-17); // only 17 years old
        var handler = BuildHandler(db);

        var result = await handler.Handle(new SignupAmbassadorCommand(req), CancellationToken.None);

        result.IsSuccess.Should().BeFalse();
        result.ErrorCode.Should().Be("UNDERAGE");
    }

    [Fact]
    public async Task Handle_WhenApplicantTurns18Today_Succeeds()
    {
        await using var db = InMemoryDbHelper.Create();
        await db.MembershipLevels.AddAsync(BuildLevel());
        await db.SaveChangesAsync();

        var req = BuildRequest(slug: "exactly18");
        // Born exactly 18 years ago today — just barely qualifies
        req.DateOfBirth = FixedNow.AddYears(-18);
        var handler = BuildHandler(db);
    ...
    public async Task Handle_WhenEmailAlreadyTaken_ReturnsEmailTaken()
    {
        ...
        var result = await handler.Handle(
            new SignupAmbassadorCommand(BuildRequest()), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WhenSlugAlreadyTaken_ThrowsDuplicateReplicateSiteException()
    {
        await using var db = InMemoryDbHelper.Create();
        await db.MembershipLevels.AddAsync(BuildLevel());
        var existing = BuildSponsor();
        existing.ReplicateSiteSlug = "mysite";
        await db.MemberProfiles.AddAsync(existing);
        await db.SaveChangesAsync();

        var handler = BuildHandler(db);

        var act = () => handler.Handle(
            new SignupAmbassadorCommand(BuildRequest(slug: "mysite")), CancellationToken.None);

        await act.Should().ThrowAsync<DuplicateReplicateSiteException>();
    }

    [Fact]
    public async Task Handle_WhenSlugIsNull_SkipsSlugValidation()
    {
        await using var db = InMemoryDbHelper.Create();
        await db.MembershipLevels.AddAsync(BuildLevel());
        await db.SaveChangesAsync();
        ...
        var handler = BuildHandler(db);
        var req = BuildRequest(slug: null);
    ...
    public async Task Handle_WhenMembershipLevelNotFound_ReturnsMembershipLevelNotFound()
    {
        ...
        var result = await handler.Handle(
            new SignupAmbassadorCommand(BuildRequest(membershipLevelId: 99)), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WhenValid_CreatesMemberProfileWithAmbassadorType()
    {
        await using var db = InMemoryDbHelper.Create();
        await db.MembershipLevels.AddAsync(BuildLevel());
        await db.SaveChangesAsync();
        ...
        member!.MemberType.Should().Be(MemberType.Ambassador);
        member.Status.Should().Be(MemberAccountStatus.Pending);
        member.Email.Should().Be("new.ambassador@example.com");
    }
    ...
    [Fact]
    public async Task Handle_WhenValid_CreatesGenealogyNodeWithCorrectPath()
    {
        await using var db = InMemoryDbHelper.Create();
        await db.MembershipLevels.AddAsync(BuildLevel());
        await db.SaveChangesAsync();
    ...
    [Fact]
    public async Task Handle_WhenValid_ReturnsAmbassadorMemberTypeInResponse()
    {
        await using var db = InMemoryDbHelper.Create();
        await db.MembershipLevels.AddAsync(BuildLevel());
        await db.SaveChangesAsync();
    ...
    [Fact]
    public async Task Handle_WhenValid_ReturnsSignupIdThatMatchesPendingOrder()
    {
        await using var db = InMemoryDbHelper.Create();
        await db.MembershipLevels.AddAsync(BuildLevel());
        await db.SaveChangesAsync();
        ...
        var orderExists = await db.Orders.AnyAsync(o => o.Id == result.Value!.SignupId);
    ...
    public async Task Handle_WhenIdentityCreateFails_ReturnsIdentityCreateFailed()
    {
        ...
        var result = await handler.Handle(
            new SignupAmbassadorCommand(BuildRequest(slug: "failtest")), CancellationToken.None);
    ...
    [Fact]
    public async Task Handle_WhenValid_AssignsAmbassadorRole()
    {
        await using var db = InMemoryDbHelper.Create();
        await db.MembershipLevels.AddAsync(BuildLevel());
        await db.SaveChangesAsync();
        ...
        mgr.Verify(u => u.AddToRoleAsync(
            It.IsAny<ApplicationUser>(), "Ambassador"), Times.Once);
    }
